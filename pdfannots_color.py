...
import os, sys, io, textwrap, argparse
...
class PrettyPrinter:
    ...
    def format_annot(self, annot, extra=None):
        ...
        if (text and len(text) == 1 and len(text[0].split()) <= 0 # words
    ...
    def printall_grouped(self, sections, annots, outfile):
        ...
        for secname in sections:
            if highlights_g and secname == 'highlights_g':
                ...
                for i,a in enumerate(highlights_g):
                    print(self.format_annot(a))
                    f = open(outpath+"pos/p_"+args.input.name[args.input.name.rfind('/')+1:]+"_"+str(i)+".txt",'w')
                    f.write(self.format_annot(a))
                    f.close()
            if highlights_r and secname == 'highlights_r':
                printheader("Highlights_r")
                for i,a in enumerate(highlights_r):
                    print(self.format_annot(a))
                    f = open(outpath+"neg/n_"+args.input.name[args.input.name.rfind('/')+1:]+"_"+str(i)+".txt",'w')
                    f.write(self.format_annot(a))
                    f.close()
'''
...
def parse_args(inp):
    ...
    #p.add_argument("-input", default='test3.pdf',metavar="INFILE", type=argparse.FileType("rb"),
    #               help="PDF file to process")
    p.add_argument("-input", default=inp, metavar="INFILE", type=argparse.FileType("rb"),
                   help="PDF file to process")
